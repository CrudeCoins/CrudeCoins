...
#if defined(NDEBUG)
# error "CrudeCoins cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc4dd5c6480780aeb15c9288cff538b8853b1c9b61a47ac61df816d6c72ee2485");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CrudeCoins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CrudeCoins Signed Message:\n";
...
{
    // CrudeCoins: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 19250000 * COIN;
            }

    // Subsidy is cut in half every 770000 blocks
    nSubsidy >>= (nHeight / 770000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CrudeCoins: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CrudeCoins: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1516108226, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "CNN Jan 16 2016 Trumps first year of diplomacy finds friends and foes";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0448ebd8844b3fd9b8184dd87127b76d600a0b3e5b85ee630053212dcad5f519d61c30387aa95cc1faf4cadbeaa13bd6e1439ff548409b9badc60804b67b581eaf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516108226;
        ...
        assert(block.hashMerkleRoot == uint256("0xec100ff284bd7f4d726326371a7bb0605bbab5318adf88b04573c326998a3a6f"));
...
unsigned char pchMessageStart[4] = { 0xef, 0x05, 0x70, 0x22 }; // CrudeCoins: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrudeCoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrudeCoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrudeCoinsMiner(CWallet *pwallet)
{
    printf("CrudeCoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crudecoins-miner");
    ...
    try { loop {
        ...
        printf("Running CrudeCoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrudeCoinsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrudeCoinsMiner, pwallet));
}
